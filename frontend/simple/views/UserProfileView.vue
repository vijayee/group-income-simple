  ...
  methods: {
    async save () {
      try {
        ...
        for (let key of Object.keys(this.edited)) {
          ...
          steps.push({ execute: 'contracts/identity/setAttribute', description: `Set ${key} attribute`, args: { contractId: 'identityContractId', name: `${key}AttributeName`, value: `${key}AttributeValue` } })
        }

        await sbp('transactions/run', 'Set Profile Attributes', true, steps)
    ...
    async saveGroupProfile () {
      try {
        ...
        await sbp('transactions/run', 'Save Group Profile', true, [
          { execute: 'setInScope',
            args: {
          ...
          {
            execute: 'contracts/group/saveGroupProfile',
